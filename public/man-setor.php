...
<head>
     ...
     <title>Setores - Gerenciamento de Colaboradores</title>
</head>
...
$(document).ready(function() {
     $('#tab-0').DataTable({
          ...
          "language": {
               ...
               "infoEmpty": "Sem registros de setores ...",
               "sSearch": "Buscar:",
...
<?php
     ...
     if ($_SESSION['wrkopereg'] == 3) { 
          $bot = 'Deletar'; 
          $del = "cor-2";
          $per = ' onclick="return confirm(\'Confirma exclusão de setor informada em tela ?\')" ';
     }  
     ...
     if (isset($_REQUEST['salvar']) == true) {
          if ($_SESSION['wrkopereg'] == 1) {
               ...
               if ($ret == 0) {
                    $ret = incluir_dad();
                    $ret = gravar_log(11,"Inclusão de nova setor: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
                    exit('<script>location.href = "man-setor.php?ope=1&cod=0"</script>'); 
               }
          }
          if ($_SESSION['wrkopereg'] == 2) {
               ...
               if ($ret == 0) {
                    $ret = alterar_dad();
                    $ret = gravar_log(12,"Alteração de setor existente: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
                    exit('<script>location.href = "man-setor.php?ope=1&cod=0"</script>'); 
               }
          }
          if ($_SESSION['wrkopereg'] == 3) {
               $ret = excluir_dad();
               $ret = gravar_log(13,"Exclusão de setor existente: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
               exit('<script>location.href = "man-setor.php?ope=1&cod=0"</script>'); 
          }
     }
     ...
     <div class="container-fluid">
          <div class="row">
               ...
               <div class="col-md-10">
                    ...
                    <div class="row">
                         <div class="col-md-10">
                              <h3 class="cor-4"><strong>Manutenção de Setores</strong></h3>
                         </div>
                         ...
                         <div class="col-md-1 text-center">
                              <a href="man-setor.php?ope=1&cod=0"
                                   title="Abre página para adicionar (criar) mais uma setor dentro do sistema.">
                                   <i class="fa fa-plus-circle fa-2x" aria-hidden="true"></i>
                              </a>
                         </div>
                    </div>
                    <br />
                    <form class="qua-2" id="tel_c" name="frmTelMan" action="man-setor.php" method="POST">
                         <div class="row">
                              <div class="col-md-2">
                                   <label>Código</label>
...
function consiste_dad() {
     ...
     if (trim($_REQUEST['des']) == "") {
          echo '<script>alert("Descrição do Setor não pode estar em branco");</script>';
          return 1;
     }
     return $sta;
}
...
function ler_dados(&$cha) {
     ...
     if ($nro == 0 || $reg == false) {
          echo '<script>alert("Código do Setor informada não cadastrada");</script>';
          $nro = 1;
     } else {
...
function carrega_dad() {
     $ret = 0;
     include_once "dados.php";
     $com = "Select * from tb_dados where dadtipo = 2 order by daddescricao, iddados";
     $nro = leitura_reg($com, $reg);
     foreach ($reg as $lin) {
          $txt =  '<tr>';
          $txt .= '<td class="text-center"><a href="man-setor.php?ope=2&cod=' . $lin['iddados'] . '" title="Efetua alteração do registro informado na linha"><i class="large material-icons">healing</i></a></td>';
          $txt .= '<td class="text-center"><a href="man-setor.php?ope=3&cod=' . $lin['iddados'] . '" title="Efetua alteração do registro informado na linha"><i class="cor-1 large material-icons">delete_forever</i></a></td>';
