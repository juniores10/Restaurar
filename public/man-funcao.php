...
<head>
     ...
     <title>Funções - Gerenciamento de Colaboradores</title>
</head>
...
<?php
     ...
     if ($_SESSION['wrkopereg'] == 3) { 
          $bot = 'Deletar'; 
          $del = "cor-2";
          $per = ' onclick="return confirm(\'Confirma exclusão de função informada em tela ?\')" ';
     }  
     ...
     if (isset($_REQUEST['salvar']) == true) {
          if ($_SESSION['wrkopereg'] == 1) {
               ...
               if ($ret == 0) {
                    $ret = incluir_dad();
                    $ret = gravar_log(11,"Inclusão de nova função: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
                    exit('<script>location.href = "man-funcao.php?ope=1&cod=0"</script>'); 
               }
          }
          if ($_SESSION['wrkopereg'] == 2) {
               ...
               if ($ret == 0) {
                    $ret = alterar_dad();
                    $ret = gravar_log(12,"Alteração de função existente: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
                    exit('<script>location.href = "man-funcao.php?ope=1&cod=0"</script>'); 
               }
          }
          if ($_SESSION['wrkopereg'] == 3) {
               $ret = excluir_dad();
               $ret = gravar_log(13,"Exclusão de função existente: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
               exit('<script>location.href = "man-funcao.php?ope=1&cod=0"</script>'); 
          }
     }
     ...
     <div class="container-fluid">
          <div class="row">
               ...
               <div class="col-md-10">
                    ...
                    <div class="row">
                         <div class="col-md-10">
                              <h3 class="cor-4"><strong>Manutenção de Funções</strong></h3>
                         </div>
                         ...
                         <div class="col-md-1 text-center">
                              <a href="man-funcao.php?ope=1&cod=0"
                                   title="Abre página para adicionar (criar) mais uma função dentro do sistema.">
                                   <i class="fa fa-plus-circle fa-2x" aria-hidden="true"></i>
                              </a>
                         </div>
                    </div>
                    <br />
                    <form class="qua-2" id="tel_c" name="frmTelMan" action="man-funcao.php" method="POST">
                         <div class="row">
                              <div class="col-md-2">
                                   <label>Código</label>
...
function ultimo_cod() {
     $cod = 1;
     include_once "dados.php";
     $nro = acessa_reg('Select iddados from tb_dados order by iddados desc Limit 1', $reg);
     if ($nro == 1) {
          $cod = $reg['iddados'] + 1;
     }        
     return $cod;
...
function consiste_dad() {
     ...
     if (trim($_REQUEST['des']) == "") {
          echo '<script>alert("Descrição da Função não pode estar em branco");</script>';
          return 1;
     }
     return $sta;
}
...
function ler_dados(&$cha) {
     include_once "dados.php";
     $nro = acessa_reg('Select * from tb_dados where iddados = ' . $cha, $reg);
     if ($nro == 0 || $reg == false) {
          echo '<script>alert("Código da Função informada não cadastrada");</script>';
          $nro = 1;
     } else {
          $_REQUEST['sta'] = $reg['dadstatus'];
          $_REQUEST['set'] = $reg['dadcodigo'];
          $_REQUEST['des'] = $reg['daddescricao'];
     }
     return $cha;
}
...
function carrega_dad() {
     $ret = 0;
     include_once "dados.php";
     $com = "Select * from tb_dados  where dadtipo = 1 order by daddescricao, iddados";
     $nro = leitura_reg($com, $reg);
     foreach ($reg as $lin) {
          $txt =  '<tr>';
          $txt .= '<td class="text-center"><a href="man-funcao.php?ope=2&cod=' . $lin['iddados'] . '" title="Efetua alteração do registro informado na linha"><i class="large material-icons">healing</i></a></td>';
          $txt .= '<td class="text-center"><a href="man-funcao.php?ope=3&cod=' . $lin['iddados'] . '" title="Efetua alteração do registro informado na linha"><i class="cor-1 large material-icons">delete_forever</i></a></td>';
          $txt .= '<td class="text-center">' . $lin['iddados'] . '</td>';
          if ($lin['dadstatus'] == 0) {$txt .= "<td>" . "Normal" . "</td>";}
          if ($lin['dadstatus'] == 1) {$txt .= "<td>" . "Bloqueado" . "</td>";}
          if ($lin['dadstatus'] == 2) {$txt .= "<td>" . "Suspenso" . "</td>";}
          if ($lin['dadstatus'] == 3) {$txt .= "<td>" . "Cancelado" . "</td>";}
          $txt .= '<td>' . retorna_dad('daddescricao', 'tb_dados', 'iddados', $lin['dadcodigo']) . '</td>';
          $txt .= '<td>' . $lin['daddescricao'] . '</td>';
...
function incluir_dad() {
     ...
     $sql .= "'" . substr(trim($_REQUEST['des']),0,25) . "',";
 ...
 function alterar_dad() {
     $ret = 0;
     include_once "dados.php";
     $sql  = "update tb_dados set ";
     $sql .= "dadstatus = '". $_REQUEST['sta'] . "', ";
     $sql .= "dadcodigo = '". $_REQUEST['set'] . "', ";
     $sql .= "daddescricao = '". trim($_REQUEST['des']) . "', ";
     $sql .= "dadresumida = '". substr(trim($_REQUEST['des']), 0, 25) . "', ";
     $sql .= "keyalt = '" . $_SESSION['wrkideusu'] . "', ";
     $sql .= "datalt = '" . date("Y/m/d H:i:s") . "' ";
     $sql .= "where iddados = " . $_SESSION['wrkcodreg'];
     $ret = comando_tab($sql, $nro, $ult, $men);
     if ($ret == false) {
          print_r($sql);
...
function excluir_dad() {
     $ret = 0;
     include_once "dados.php";
     $sql  = "Delete from tb_dados where iddados = " . $_SESSION['wrkcodreg'] ;
