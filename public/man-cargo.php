...
$(document).ready(function() {
     $('#tab-0').DataTable({
          ...
          "language": {
               ...
               "infoEmpty": "Sem registros de cargos ...",
               "sSearch": "Buscar:",
...
<?php
     ...
     if ($_SESSION['wrkopereg'] == 3) { 
          $bot = 'Deletar'; 
          $del = "cor-2";
          $per = ' onclick="return confirm(\'Confirma exclusão de cargo informada em tela ?\')" ';
     }  
     ...
     if (isset($_REQUEST['salvar']) == true) {
          if ($_SESSION['wrkopereg'] == 1) {
               ...
               if ($ret == 0) {
                    $ret = incluir_dad();
                    $ret = gravar_log(11,"Inclusão de nova cargo: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
                    exit('<script>location.href = "man-cargo.php?ope=1&cod=0"</script>'); 
               }
          }
          if ($_SESSION['wrkopereg'] == 2) {
               ...
               if ($ret == 0) {
                    $ret = alterar_dad();
                    $ret = gravar_log(12,"Alteração de cargo existente: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
                    exit('<script>location.href = "man-cargo.php?ope=1&cod=0"</script>'); 
               }
          }
          if ($_SESSION['wrkopereg'] == 3) {
               $ret = excluir_dad();
               $ret = gravar_log(13,"Exclusão de cargo existente: " . (isset($_REQUEST['nom']) == false ? "" : $_REQUEST['nom'])); $_SESSION['wrkcodreg'] = ultimo_cod();
               exit('<script>location.href = "man-cargo.php?ope=1&cod=0"</script>'); 
          }
     }
     ...
     <div class="container-fluid">
          <div class="row">
               ...
               <div class="col-md-10">
                    ...
                    <div class="row">
                         <div class="col-md-10">
                              <h3 class="cor-4"><strong>Manutenção de Cargos</strong></h3>
                         </div>
                         <div class="col-md-1 text-center">
                              <a id="mos_t" href="#"
                                   title="Mostra e esconde a página detalhada com os dados de cargos para edição.">
                                   <i class="fa fa-eye fa-2x" aria-hidden="true"></i>
                              </a>
                         </div>
                         <div class="col-md-1 text-center">
                              <a href="man-cargo.php?ope=1&cod=0"
                                   title="Abre página para adicionar (criar) mais um cargo dentro do sistema.">
                                   <i class="fa fa-plus-circle fa-2x" aria-hidden="true"></i>
                              </a>
                         </div>
                    </div>
                    <br />
                    <form class="qua-2" id="tel_c" name="frmTelMan" action="man-cargo.php" method="POST">
                         <div class="row">
                              <div class="col-md-2">
                                   <label>Código</label>
...
function consiste_dad() {
     ...
     if (trim($_REQUEST['des']) == "") {
          echo '<script>alert("Descrição do Cargo não pode estar em branco");</script>';
          return 1;
     }
     return $sta;
}
...
function carrega_dad() {
     $ret = 0;
     include_once "dados.php";
     $com = "Select * from tb_dados where dadtipo = 3 order by daddescricao, iddados";
     $nro = leitura_reg($com, $reg);
     foreach ($reg as $lin) {
          $txt =  '<tr>';
          $txt .= '<td class="text-center"><a href="man-cargo.php?ope=2&cod=' . $lin['iddados'] . '" title="Efetua alteração do registro informado na linha"><i class="large material-icons">healing</i></a></td>';
          $txt .= '<td class="text-center"><a href="man-cargo.php?ope=3&cod=' . $lin['iddados'] . '" title="Efetua alteração do registro informado na linha"><i class="cor-1 large material-icons">delete_forever</i></a></td>';
