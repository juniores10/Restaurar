/*
  ...
  1. New Tables
    - `expedition_records`
      - `id` (uuid, primary key)
      - `order_code` (text) - Codigo do Pedido
      - `boxes` (integer) - Caixas
  ...
  3. Notes
    - This table stores shipping/expedition records
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated users can read expedition records' AND tablename = 'expedition_records') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admin users can insert expedition records' AND tablename = 'expedition_records') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admin users can update expedition records' AND tablename = 'expedition_records') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admin users can delete expedition records' AND tablename = 'expedition_records') THEN
